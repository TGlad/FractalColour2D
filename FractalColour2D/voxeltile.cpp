...
int _tmain(int argc, _TCHAR* argv[])
{
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    for (int j = 0; j < 4; j++)
    {
      Vector3d pos = Vector3d(n,n,n)/2.0 - 0.5*(double)n * sides[i][j];
  ...
  for (int x = 0; x < n; x++)
    for (int y = 0; y < n; y++)
      for (int z = 0; z < n; z++)
      {
 //       grid[x][y][z] = (x + y + z == 6) ? 1 : 0;
 //       if (!(x==2 && y==2 && z==2))
        if ((x + y + z) != 10 && (x + y + z) != 11)
          continue;
        for (int i = 0; i < 3; i++)
        {
          vector<Vector2d> list;
  ...
  saveSVG("voxeltile2.svg", lists, colours);
}
