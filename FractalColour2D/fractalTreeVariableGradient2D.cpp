...
void Section::draw(ofstream &svg, const Vector2d &origin, const Vector2d &xAx, const Vector2d &yAx)
{
  ...
  svg << "\" fill=\"black\" stroke=\"black\" stroke-width=\"0\" />\n"; // stroke=\"black\"
...
#define SIMPLE
#if defined SIMPLE
static const double exponent = 2.0; // for some reason only works with exponent=2
double getWidth(double length)
{
  return pow(1.0 - length, exponent);
}
double getWidthDot(double length)
{
  return exponent * pow(1.0 - length, exponent - 1.0);
}
#else
double getWidth(double length)
{
  return sqrt(1.0 - sqr(length));
}
double getWidthDot(double length)
{
  return -length / sqrt(1.0 - sqr(length));
}
#endif

static double cantorScale = 1.0/3.0;// 3.0 / 5.0;
static double grad = 0.0; // distribute evenly, 0 to 1
static double p = 1.5; // 1 to 1.58 when c=1/3. 1 to 1.27 when n=2 //  distribute: 1.05, 1.25, 1.5, 2.0, 2.5
void Section::split()
{
  double minLength = 0.0055;
  if (length <= minLength)
    return;
  Section child1, child2;

  // we're splitting this section in half, so what is the gradient at this mid point?
  double midH = h + length / 2.0;
  double gradient = getWidthDot(midH);
  //  double gradient = (1.0 - 2.0*pow((1.0 - c) / 2.0, 1.0 / p)) / (0.5*pow(c, 1.0 / p));
  // find p
  p = 1.5;
  for (int a = 0; a < 15; a++)
    p = log((1.0 - cantorScale) / 2.0) / log(0.5 - gradient * 0.25*pow(cantorScale, 1.0 / p));
  double gradCheck = (1.0 - 2.0*pow((1.0 - cantorScale) / 2.0, 1.0 / p)) / (0.5*pow(cantorScale, 1.0 / p));
  if ((gradCheck - gradient) > 0.01)
    cout << "bad convergence" << endl;
  double midWidth = (getWidth(h) + getWidth(h + length)) / 2.0;
  double widthScale = getWidth(midH) / midWidth;
  ...
  double a = widthScale * pow(areaLoss, 1.0 / p);
  double b = widthScale * pow((w2 + w2b - areaLoss) / 2.0, 1.0 / p);
  double c = widthScale * pow((w2 + w2b + areaLoss) / 2.0, 1.0 / p);
