...
void Section::draw(ofstream &svg, const Vector2d &origin, const Vector2d &xAx, const Vector2d &yAx)
{
  ...
  Vector2d corners[] = { start - x*width*0.5, start + x*width*0.5, start + x*0.5*width2 + y*length, start - x*0.5*width2 + y*length };
...
int _tmain(int argc, _TCHAR* argv[])
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    base.split(i);
    stringstream stream;
    stream << "tree_iterations_" << i << ".svg";
    saveSVG(stream.str(), base);
  }
}
